...
func (s *Server) newHTTPServer(tlsCredentials credentials.TransportCredentials, appPort string) {
	...
	if err != nil {
		log.Panicf("Failed to dial: %w", err)
	}
	defer func() {
		if err != nil {
	...
	if err = authpb.RegisterAuthServiceHandler(s.ctx, s.httpServer, conn); err != nil {
		log.Panicf("proto.RegisterAuthServiceHandler(): %w", err)
	}
}
...
func CustomErrorHandler(
	...
	if !ok {
		log.Errorf("runtime.RPCMethod(): %w", err)
	}
	log.Info().Str("method", val).Msg("CustomErrorHandler")
