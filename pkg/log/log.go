...
func InitLogger(cfg *Config, opts ...zap.Option) (*zap.Logger, *ZapProperties, error) {
	...
	if len(cfg.File.Filename) > 0 {
		...
		output = append(output, zapcore.AddSync(lg))
	}

	var errOutput zapcore.WriteSyncer
	return InitLoggerWithWriteSyncer(cfg, zapcore.NewMultiWriteSyncer(output...), errOutput, opts...)
}
...
func InitLoggerWithWriteSyncer(cfg *Config, output, errOutput zapcore.WriteSyncer, opts ...zap.Option) (
	*zap.Logger, *ZapProperties, error,
) {
